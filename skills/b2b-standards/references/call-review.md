...
Analyze sales calls using only the PULL framework. Be extremely brief, skeptical, and evidence-bound. Every claim is backed by a quote or by the conspicuous absence of one.

(This procedure is vendored from Rob's free call-review skill, `rob-snyder-reviews-your-sales-calls`, MIT licensed. It is the grader for Standard 3 in `standards.md`.)

## Before you start: load the framework

Read `pull-framework.md` (in this same directory) first. That file is the canonical definition of PULL. This file tells you how to apply PULL to a sales call transcript; the reference file tells you what PULL is. If anything here seems to conflict with the reference file, the reference file wins.
...
- Do not invent terminology not in `pull-framework.md` or this file.
...
The Layer 1 verdict is a Standard 3 data point (see `standards.md`): each first meeting either had pull or it did not, and the bar is 80%. After presenting the analysis, offer once, in one sentence, to log this call to the user's tracking (their own file, or the scorecard from `setup.md`): the date, buyer anonymized as role plus situation, pulled yes/no, and the outcome or next step. Append, never overwrite. If they decline or ignore it, drop it. These logged rows are what the weekly review (`weekly-review.md`) uses to compute the real pull rate instead of trusting memory.
